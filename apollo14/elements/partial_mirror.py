...
- **Tabulated** — pass ``reflectance`` directly (shape ``(K,)``); the
  mirror stores it verbatim on the ``wavelengths`` grid.
- **Parametric** — pass a :class:`apollo14.spectral.SumOfGaussiansCurve`
  (or any pytree exposing ``sample(wavelengths) -> (K,)``) via
  ``curve``. The mirror eagerly samples it onto ``wavelengths`` in
  ``__post_init__`` and stores the result; the tracer never sees the
  curve. Gradients flow through the sampled values back to the curve's
  parameters via JAX's pytree machinery.

This split keeps the parametrization (Gaussians, splines, raw samples)
out of the mirror itself — adding a new curve type doesn't touch this
file.
"""
...
class PartialMirror(PlanarElement):
    ...
    curve: object = None              # optional SpectralCurve; eager-sampled
    ...
    def __post_init__(self):
        ...
        if self.curve is not None:
            # Eager-sample: parametric curve → fixed (K,) array. Gradients
            # still reach the curve's leaves via the sampled values.
            self.reflectance = self.curve.sample(self.wavelengths)
        elif self.reflectance is None:
