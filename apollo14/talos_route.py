"""Talos-specific route — typed element states for the Talos combiner layout.

Uses element-specific state types (MirrorState, RefractState, etc.) with
hard-coded preamble/mirror/branch structure. Used by ``talos_trace.py``.

For the generic route with uniform SurfaceState, see ``route.py``.
"""
...
class TalosRoute(NamedTuple):
    """Talos-specific sequential optical path with typed element states.
...
def display_route(system: OpticalSystem, wavelength: float,
                  entry_face: str = "back", exit_face: str = "top") -> TalosRoute:
    """Build the Talos display path: projector → mirrors → pupil.
    ...
    Returns:
        TalosRoute ready for tracing with ``talos_trace.trace_ray`` etc.
    """
