...
def export_zemax_bundle(
    ...
    **prescription_kwargs,
    ...
    if coating_blocks:
        (output_dir / coating_file).write_text(format_coating_file(
    ...
    (output_dir / "README.md").write_text(_readme(
        prescription, fits, resolved_glass_names, glass_catalog, coating_file,
...
def _readme(prescription, fits, glass_names, glass_catalog, coating_file,
    ...
    return f"""# Apollo14 → Zemax OpticStudio bundle
...
1. `{glass_catalog}.AGF` → the **Glass catalog** folder (`Glasscat`).
2. `{coating_file}` → the **Coating** folder (`Coatings`).
